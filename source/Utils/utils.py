...
class Helper:
    def __init__(self, config_path):
        ...
        self.model_path = self.file_path['model_path']
        ...
        # vocabulary
        self.src_voc, self.trc_voc = self.get_voca()

        # print step
        self.train_step_print = self.basic_parameter['train_step_print']
        self.val_step_print = self.basic_parameter['val_step_print']
        self.step_save = self.basic_parameter['step_save']

    def encoder_parameter(self):
        if self.nmt_model == 'seq2seq':
            param = {
                'embedding_size': self.basic_parameter['encoder_vocab_size'],
                'embedding_dim': self.seq2seq['embedding_dim'],
                'pad_id': self.src_voc['<pad>'],
                ...
                'rnn_dropout': self.seq2seq['encoder']['encoder_rnn_dropout'],
            ...
            param = {
                'input_dim': self.basic_parameter['encoder_vocab_size'],
    ...
    def decoder_parameter(self):
        if self.nmt_model == 'seq2seq':
            ...
            param = {
                'input_dim': self.basic_parameter['decoder_vocab_size'],
    ...
    def get_voca(self):
        try:  # vocabulary 불러오기
            src_voc, tar_voc = create_or_get_voca(save_path=self.file_path['dictionary_path'],
                                                  src_vocab_size=self.basic_parameter['encoder_vocab_size'],
                                                  trc_vocab_size=self.basic_parameter['decoder_vocab_size'],
                                                  src_region=self.basic_parameter['src_region'],
                                                  trc_region=self.basic_parameter['trc_region'])
        except OSError:  # 경로 error 발생 시 각각의 경로를 입력해서 가지고 오기
            src_voc, tar_voc = create_or_get_voca(save_path=self.file_path['dictionary_path'],
                                                  src_corpus_path=self.x_train_path,
                                                  trc_corpus_path=self.y_train_path,
                                                  src_vocab_size=self.basic_parameter['encoder_vocab_size'],
                                                  trc_vocab_size=self.basic_parameter['decoder_vocab_size'],
                                                  src_region=self.basic_parameter['src_region'],
                                                  trc_region=self.basic_parameter['trc_region'])
        return src_voc, tar_voc
